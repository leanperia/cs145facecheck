...
def RunInference(request):
    webagent = EndAgent.objects.get(pk=2)
...
def EditInference(request, pk):
    ...
    agent = EndAgent.objects.get(pk=instance.endagent.id)
    mv = MLModelVersion.objects.all().order_by('-time_trained')[0]
...
def RetrainMLmodel(request):
    mv = MLModelVersion.objects.all().order_by('-time_trained')[0]
    mv.fa_rate = mv.fp_count/mv.inf_count
    mv.fr_rate = mv.fn_count/mv.inf_count

    KNN_CLASSIFIER = generate_knn_model(os.path.join(MEDIA_ROOT,'samplephotos/'),request.POST['k'])
    ...
    with open('media/knnclf_init.json') as f:
