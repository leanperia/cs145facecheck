...
from facecheck.settings import BASE_DIR, MEDIA_URL, MEDIA_ROOT
...
def RunInference(request):
    ...
    if knn_classifier == None:
        ...
        knn_classifier = joblib.load(urlopen(MEDIA_URL+str(mv.model.name)))
...
def RetrainMLmodel(request):
    ...
    new_clf = generate_knn_model(int(request.POST['k']),
